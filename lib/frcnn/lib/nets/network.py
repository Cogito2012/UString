...
class Network(object):
  ...
  def _roi_align(self, bottom, rois, name):
    """
    ...
    with tf.variable_scope(name) as scope:
      ...
      boxes = tf.slice(rois, [0, 1], [-1, 4], name="boxes")
  ...
  def crop_and_resize(self, image, boxes, box_ind, crop_size, pad_border=True):
    ...
    if pad_border:
      ...
      image = tf.transpose(image, [0, 3, 2, 1])  # to NCHW
      image = tf.pad(image, [[0, 0], [0, 0], [1, 1], [1, 1]], mode='SYMMETRIC')  # only support for the data_format=='NCHW'
      image = tf.transpose(image, [0, 3, 2, 1])  # to NHWC
      boxes = boxes + 1
  ...
  def _build_network(self, is_training=True):
    ...
    with tf.variable_scope(self._scope, self._scope):
      ...
      if cfg.POOLING_MODE == 'crop':
        ...
        pool5 = self._roi_align(net_conv, rois, "pool5")
      else:
  ...
  # only useful during testing mode
  def test_image(self, sess, image, im_info, with_feat=False):
    feed_dict = {self._image: image,
                 self._im_info: im_info}
    output_dict = [self._predictions["cls_score"],
                  self._predictions['cls_prob'],
                  self._predictions['bbox_pred'],
                  self._predictions['rois']]
    head, fc7 = None, None
    if with_feat:
      output_dict.append(self._layers['head'])
      output_dict.append(self._layers['fc7'])
      cls_score, cls_prob, bbox_pred, rois, head, fc7 = sess.run(output_dict, feed_dict=feed_dict)
    else:
      cls_score, cls_prob, bbox_pred, rois = sess.run(output_dict, feed_dict=feed_dict)

    return cls_score, cls_prob, bbox_pred, rois, head, fc7
