...
class I3Res50(nn.Module):
    VALID_ENDPOINTS = ('conv1', 'bn1', 'relu1', 'maxpool1', 'block1', 'maxpool2', 'block2', 'block3', 'block4', 'avgpool', 'fc')
    ...
    def __init__(self, block=Bottleneck, layers=[3, 4, 6, 3], num_classes=400, use_nl=False):
        ...
        self.conv1 = nn.Conv3d(3, 64, kernel_size=(5, 7, 7), stride=(1, 2, 2), padding=(2, 3, 3), bias=False)
        ...
        self.layer4 = self._make_layer(block, 512, layers[3], stride=1, temp_conv=[0, 1, 0], temp_stride=[1, 1, 1])
    ...
    def extract_features(self, x):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)       # 112x112, stride=2
        x = self.maxpool1(x)   # 55x55, stride=4

        x = self.layer1(x)     # 55x55, stride=4
        x = self.maxpool2(x)   # 55x55, stride=4
        x = self.layer2(x)     # 28x28, stride=8
        x = self.layer3(x)     # 14x14, stride=16
        x = self.layer4(x)     # 7x7,   stride=32
...
def i3_res50(num_classes, model_file=None):
    if model_file is None:
        model_file = 'models_i3d/i3d_r50_kinetics.pth'
...
def i3_res50_nl(num_classes, model_file=None, use_cuda=True):
    if model_file is None:
...
if __name__=='__main__':
    ...
    # pred, losses = net(inp)
    features, feats_pool = net.extract_features(inp['frames'])